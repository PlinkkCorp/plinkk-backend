<script lang="ts">
	...
	export let user: any;
	export let plinkk: any = null;
	export let plinkkCount: number = 0;

	$: roleName = user && user.role ? (user.role.id || user.role.name) : null;
	$: isPremiumUser = !!(user && (user.isPremium || user.isPartner || (roleName && ['ADMIN','DEVELOPER','MODERATOR'].includes(roleName))));
	$: showPremiumUi = !!(user && user.showPremiumUi !== false);
	...
	$: displayEmail = user && (user.publicEmail || user.email);
</script>
	...
	<div class="flex-1 flex flex-col bg-slate-900/50 backdrop-blur-xl border border-slate-800/50 rounded-3xl p-4 shadow-xl overflow-y-auto no-scrollbar">
		...
		<div class="flex flex-col items-center text-center mb-6 pb-6 border-b border-slate-800/50">
			...
			<h3 class="font-bold text-white text-lg">
				{user?.userName || user?.id || 'Utilisateur'}
			</h3>
			{#if isPremiumUser && showPremiumUi}
				<div class="mt-2 inline-flex items-center gap-1.5 rounded-full bg-amber-500/10 text-amber-300 border border-amber-500/30 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider">
					<svg class="w-3 h-3" viewBox="0 0 24 24" fill="currentColor">
						<path d="M12 2l2.7 5.48 6.05.88-4.38 4.27 1.03 6.02L12 15.9 6.6 18.65l1.03-6.02L3.25 8.36l6.05-.88L12 2z" />
					</svg>
					Premium
				</div>
			{:else}
				<a href="/premium" class="mt-2 group inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-violet-500/15 to-fuchsia-500/15 text-violet-300 border border-violet-500/25 px-3 py-1 text-[10px] font-bold uppercase tracking-wider hover:border-violet-500/40 hover:from-violet-500/25 hover:to-fuchsia-500/25 transition-all">
					<svg class="w-3 h-3 text-violet-400" viewBox="0 0 24 24" fill="currentColor">
						<path d="M12 2l2.7 5.48 6.05.88-4.38 4.27 1.03 6.02L12 15.9 6.6 18.65l1.03-6.02L3.25 8.36l6.05-.88L12 2z" />
					</svg>
					Upgrade →
				</a>
		...
		<nav class="flex-1 space-y-6">
			...
			<div class="space-y-1">
				...
				<a href="/" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-200 {isActive('/') ? 'bg-violet-500/10 text-violet-400 border border-violet-500/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}">
					<svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
						<path d="M10 2L2 8v10a1 1 0 001 1h5V11h4v8h5a1 1 0 001-1V8l-8-6z" />
					</svg>
				...
				<a href="/stats" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-200 {isActive('/stats') ? 'bg-violet-500/10 text-violet-400 border border-violet-500/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}">
					<svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
						<path d="M3 13h2v4H3v-4zm4-6h2v10H7V7zm4 4h2v6h-2v-6zm4-8h2v14h-2V3z" />
					</svg>
					<span class="font-medium">Statistiques</span>
				</a>
				<a href="/partners" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-200 {isActive('/partners') ? 'bg-violet-500/10 text-violet-400 border border-violet-500/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}">
					<svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
			...
			<div class="space-y-1">
				...
				<a href="/plinkks" class="flex items-center justify-between px-4 py-2.5 rounded-xl transition-all duration-200 {isActive('/plinkks') ? 'bg-violet-500/10 text-violet-400 border border-violet-500/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}">
					...
					{#if plinkkCount > 0}
						<span class="bg-slate-800 text-slate-300 text-[10px] font-bold px-2 py-0.5 rounded-full border border-slate-700">
							{plinkkCount}
						</span>
					{/if}
				</a>
