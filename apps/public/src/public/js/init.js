...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function renderPlinkk(profileData) {
    ...
    if (themes.length > 0) {
      createToggleThemeButton(
        themes[profileData.selectedThemeIndex % themes.length],
      );
    }
    ...
    if (animations && animations.length) {
      ...
      applyAnimation(
        animations[profileData.selectedAnimationIndex % animations.length],
        !!profileData.EnableAnimationArticle,
      );
      applyAnimationButton(
        animations[
          profileData.selectedAnimationButtonIndex % animations.length
        ],
        !!profileData.EnableAnimationButton,
        profileData.delayAnimationButton,
      );
    }
  }
