...
export function plinkkPagesRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/plinkks", async (request, reply) => {
    ...
    const me = await prisma.user.findUnique({ where: { id: userId }, select: { id: true, role: true, userName: true, image: true } });
    if (!me) return reply.redirect(`/login?returnTo=${encodeURIComponent(String(request.raw.url || '/dashboard/plinkks'))}`);
  const pages = await prisma.plinkk.findMany({ where: { userId: me.id }, include: { settings: true }, orderBy: [{ isDefault: 'desc' }, { index: 'asc' }, { createdAt: 'asc' }] });
  ...
  return reply.view("dashboard/user/plinkks.ejs", { user: me, userId: me.id, userName: me.userName, pages: pagesForView, plinkks: pagesForView, maxPages });
  });

  // New form
  fastify.get("/plinkks/new", async (request, reply) => {
    const userId = request.session.get("data") as string | undefined;
    if (!userId) return reply.redirect(`/login?returnTo=${encodeURIComponent('/dashboard/plinkks/new')}`);
    const me = await prisma.user.findUnique({ where: { id: userId }, select: { id: true, role: true, userName: true } });
    if (!me) return reply.redirect(`/login?returnTo=${encodeURIComponent(String(request.raw.url || '/dashboard/plinkks/new'))}`);
    const pagesCount = await prisma.plinkk.count({ where: { userId: me.id } });
    const maxPages = getMaxPagesForRole(me.role);
    const canCreate = pagesCount < maxPages;
    return reply.view("dashboard/user/plinkks_new.ejs", { userName: me.userName, canCreate, maxPages });
  });

  // Create
  fastify.post<{ Body: { title: string; slug?: string; visibility?: string; isActive?: string } }>("/plinkks", async (request, reply) => {
    const userId = request.session.get("data") as string | undefined;
    if (!userId) return reply.redirect(`/login?returnTo=${encodeURIComponent('/dashboard/plinkks')}`);
    const me = await prisma.user.findUnique({ where: { id: userId }, select: { id: true, role: true } });
    if (!me) return reply.redirect(`/login?returnTo=${encodeURIComponent('/dashboard/plinkks')}`);
    ...
    await createPlinkkForUser(prisma as any, me.id, { name: title || 'Page', slugBase, visibility: visibility as any, isActive });
    return reply.redirect('/dashboard/plinkks');
  });

  // Edit: redirect to classic editor with plinkkId param
  fastify.get<{ Params: { id: string } }>("/plinkks/:id/edit", async (request, reply) => {
    const userId = request.session.get("data") as string | undefined;
  ...
  // Update
  fastify.post<{ Params: { id: string }; Body: { title?: string; slug?: string; visibility?: string; isActive?: string; isDefault?: string } }>("/plinkks/:id", async (request, reply) => {
    const userId = request.session.get("data") as string | undefined;
    if (!userId) return reply.redirect(`/login?returnTo=${encodeURIComponent('/dashboard/plinkks')}`);
    const { id } = request.params;
    const body = request.body || ({} as any);
  ...
  // Delete (soft or hard)
  fastify.post<{ Params: { id: string }; Body: { mode?: string } }>("/plinkks/:id/delete", async (request, reply) => {
    const userId = request.session.get("data") as string | undefined;
    if (!userId) return reply.redirect(`/login?returnTo=${encodeURIComponent('/dashboard/plinkks')}`);
    const { id } = request.params;
    ...
    if (mode === 'hard') {
      ...
      await prisma.plinkk.update({ where: { id }, data: { isActive: false } });
    }
    await reindexNonDefault(prisma, userId);
    return reply.redirect('/dashboard/plinkks');
  });
  ...
  fastify.post<{ Params: { id: string } }>("/plinkks/:id/set-default", async (request, reply) => {
    const userId = request.session.get("data") as string | undefined;
    if (!userId) return reply.redirect(`/login?returnTo=${encodeURIComponent('/dashboard/plinkks')}`);
    const { id } = request.params;
    const page = await prisma.plinkk.findUnique({ where: { id } });
  ...
  fastify.get<{ Params: { id: string } }>("/plinkks/:id/stats", async (request, reply) => {
    const userId = request.session.get("data") as string | undefined;
