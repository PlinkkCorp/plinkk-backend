...
export function adminStatsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/funnel", { preHandler: [requireAuth] }, async function (request, reply) {
    ...
    const [
      ...
      prisma.onboardingDraft.count({ where: { step: { gte: 1 } } }),
      prisma.onboardingDraft.count({ where: { step: { gte: 2 } } }),
      prisma.onboardingDraft.count({ where: { step: { gte: 3 } } }),
      prisma.funnelEvent.count({ where: { event: "onboarding_complete" } }),
