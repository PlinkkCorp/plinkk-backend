...
function getProfileData() {
  ...
  try {
    if (typeof window !== "undefined" && window.__PLINKK_PROFILE_DATA__)
      return window.__PLINKK_PROFILE_DATA__;
  } catch (e) {}
  return undefined;
}
...
export function applyTheme(theme) {
  ...
  document.querySelectorAll(".discord-box").forEach((box) => {
    if (
      box.classList.contains("form-box") ||
      box.classList.contains("embed-box")
    )
      return;
    const htmlBox = box;
  ...
  if (
    theme.articleHoverBoxShadow &&
    theme.articleHoverBoxShadow.trim() !== ""
  ) {
    try {
      ...
      console.warn(
        "Invalid articleHoverBoxShadow value:",
        theme.articleHoverBoxShadow,
        e,
      );
    }
  ...
  if (theme.background && theme.background.trim() !== "") {
    try {
      ...
      console.warn(
        "Invalid background value for scrollbar track:",
        theme.background,
        e,
      );
    }
  ...
  if (
    ...
    theme.buttonBackground.trim() !== "" &&
    theme.buttonHoverBackground.trim() !== "" &&
    theme.background &&
    ...
    try {
      ...
      console.warn(
        "Invalid button colors for scrollbar thumb:",
        theme.buttonBackground,
        theme.buttonHoverBackground,
        e,
      );
  ...
  if (easterEggsBtn) {
    ...
    console.log(
      "Easter eggs button not found - will be created when Easter eggs are unlocked.",
    );
  }
  if (easterEggsModal) {
    ...
    easterEggsModal.style.transition =
  ...
  const boutonsAffichageDescription =
    document.querySelectorAll(".show-desc-btn");
  ...
  if (_p.fontFamily) {
    ...
    if (articleElement)
      articleElement.style.fontFamily = `"${_p.fontFamily}", sans-serif`;

    // Dynamic Google Font loading
    const fontId =
      "google-font-" + _p.fontFamily.replace(/\s+/g, "-").toLowerCase();
...
export function setBackgroundStyles(profileData) {
  ...
  if (bgType === "video" && _p.backgroundVideo) {
    ...
    const colors = Array.isArray(_p.background)
      ? _p.background.filter(Boolean)
      : null;
    const deg =
      (Number.isFinite(_p.degBackgroundColor) ? _p.degBackgroundColor : 45) ||
      45;
...
export function applyDynamicStyles(
    ...
    try {
      if (canvaData[selectedCanvasIndex].fileNames === "matrix-effect/app.js") {
        ...
        loadScript("/canvaAnimation/matrix-effect/effect.js")
          .then(() => {
            console.log(
              "Effect.js loaded, checking window.Effect:",
              typeof window.Effect,
            );
            return loadScript("/canvaAnimation/matrix-effect/symbol.js");
          })
          .then(() => {
            console.log(
              "Symbol.js loaded, checking window.Symbol:",
              typeof window.Symbol,
            );
            return loadScript("/canvaAnimation/matrix-effect/app.js");
          })
          .then(() => {
            console.log(
              "App.js loaded, checking runCanvasAnimation:",
              typeof runCanvasAnimation,
            );
  ...
  if (
    EnableAnimationBackground &&
    !useCanvas &&
    !Array.isArray(profileData.background)
  ) {
  ...
  if (_p_neon.neonEnable === 0) {
    ...
    if (
      _p_neon.neonColors &&
      Array.isArray(_p_neon.neonColors) &&
      _p_neon.neonColors.length > 0
    ) {
      const neonGradient = _p_neon.neonColors
...
export function addEmailStyles() {
  ...
  const borderColor =
    (currentTheme === null || currentTheme === void 0
      ? void 0
      : currentTheme.buttonHoverBackground) || "#2C2F33";
  try {
    styleSheet.insertRule(
      `
            .email {
