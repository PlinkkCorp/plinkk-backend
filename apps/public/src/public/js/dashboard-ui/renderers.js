...
export function renderSocial({ container, addBtn, socials, scheduleAutoSave }) {
  ...
  if (!Array.isArray(socials) || socials.length === 0) {
    ...
    socials.forEach((s, idx) => {
      ...
      ["platform", "custom"].forEach((v) =>
        urlSourceSel.appendChild(
          el("option", {
            value: v,
            text: v === "platform" ? "Plateforme" : "Personnalisé",
          }),
        ),
      );
      const urlWrap = el("div", { class: "relative w-full md:w-1/3" });
      const urlCellWrapper = el("div", {
      ...
      ["catalog", "url", "upload"].forEach((v) => {
        const o = el("option", {
          value: v,
          text:
            v === "catalog" ? "Librairie" : v === "url" ? "Lien" : "Importer",
        });
      ...
      const iconUrlInput = el("input", {
        type: "url",
        class: "px-3 py-2 w-full rounded bg-slate-900 border border-slate-800",
      ...
      const rmCell = el("div", {
        class: "flex justify-end items-center w-full md:w-16",
      });

      function setPreviewByValue(val) {
        if (isUrlish(val)) iconPreview.src = val;
        else {
          iconPreview.src = `https://cdn.plinkk.fr/icons/${val}.svg`;
        }
      }
      ...
      function openPlatformPicker() {
        openPicker({
          ...
          renderCard: (item, i) => {
            ...
            img.className =
              "h-8 w-8 rounded bg-slate-800 border border-slate-700";
            const col = document.createElement("div");
            const title = document.createElement("div");
            title.className = "font-medium";
      ...
      const initialUrlMode =
        s.url && !/^https?:\/\//i.test(s.url) && !s.url.startsWith("/")
          ? "custom"
          : "platform";
  ...
  if (!Array.isArray(links) || links.length === 0) {
    container.append(
      emptyState({
        ...
        onAction: () => {
          links.push({ url: "https://", name: "Nouveau", text: "Link" });
          renderLinks({ container, addBtn, links, scheduleAutoSave });
          scheduleAutoSave();
        },
      }),
    );
    ...
    links.forEach((l, idx) => {
      ...
      const sourceSel = el(
        ...
        {
          class:
            "h-10 px-2 rounded bg-slate-900 border border-slate-800 text-sm w-32",
        },
        [],
      );
      ["catalog", "url", "upload"].forEach((v) => {
      ...
      themeDisplay.value =
        l.name && String(l.name).trim() !== "" ? String(l.name) : "";
      const themePickBtn = el("button", {
        class:
          "absolute right-1 top-1 h-8 px-2 text-xs rounded bg-slate-800 border border-slate-700 hover:bg-slate-700",
        text: "Choisir",
        type: "button",
        title: "Choisir le thème du bouton",
      });
      ...
      function setPreviewByValue(val) {
        if (isUrlish(val)) iconPreview.src = val;
        else iconPreview.src = val;
      }

      function openIconPickerForLink() {
        openIconModal((slug) => {
          const replaced = `https://cdn.plinkk.fr/icons/${slug}.svg`;
          icon.value = replaced;
          l.icon = replaced;
      ...
      icon.addEventListener("keydown", (e) => {
        if (e.key === "Enter" || e.key === " ") {
          e.preventDefault();
          openIconPickerForLink();
        }
      });
      iconUrlInput.addEventListener("input", () => {
        l.icon = iconUrlInput.value;
        setPreviewByValue(l.icon);
        scheduleAutoSave();
      });
      fileInput.addEventListener("change", (e) => {
      ...
      const initialSource = l.icon
        ? l.icon.startsWith("data:")
          ? "upload"
          : isUrlish(l.icon)
            ? "url"
            : "catalog"
      ...
      description.addEventListener("input", () => {
        l.description = description.value;
        scheduleAutoSave();
      });
...
export function renderLayout({
  ...
  normalized.forEach((key, idx) => {
    ...
    const row = el("div", {
      class:
        "flex items-center gap-3 w-full rounded-md border border-slate-800 bg-slate-900/50 px-2 py-2" +
    ...
    const gripBtn = el("button", {
      type: "button",
      class:
        "h-9 w-9 inline-flex items-center justify-center rounded bg-slate-800 border border-slate-700 hover:bg-slate-700",
      title: "Déplacer",
      "aria-label": "Déplacer",
    });
    gripBtn.style.cursor = "grab";
    ...
    try {
      enableDragHandle(
        ...
        container,
        order,
        () => renderLayout({ container, order, scheduleAutoSave, cfg, cfg }),
        scheduleAutoSave,
      );
    } catch {}
    container.appendChild(row);
