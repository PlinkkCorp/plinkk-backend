<script lang="ts">
	...
	$: _av_initial = _av_entity && (_av_entity.userName || _av_entity.id || _av_entity.name) 
		? String(_av_entity.userName || _av_entity.id || _av_entity.name).charAt(0).toUpperCase() 
		: 'U';
...
<div class="plinkk-av w-full h-full" class:plinkk-av--premium={_av_show} data-avatar data-avatar-user={_av_entity?.id || ''}>
	...
	<div class="plinkk-av-img bg-slate-800" class:animate-pulse={!_av_imgUrl && !_av_entity?.profileImage}>
		{#if _av_imgUrl}
			<img src={_av_imgUrl} alt="Avatar" class="h-full w-full object-cover transition-opacity duration-300" onload={(e) => e.currentTarget.classList.remove('opacity-0')} />
		{:else if _av_entity?.profileImage}
			<img src={_av_entity.profileImage} alt="Avatar" class="h-full w-full object-cover transition-opacity duration-300" onload={(e) => e.currentTarget.classList.remove('opacity-0')} />
		{:else}
			<span class="text-white font-bold">{_av_initial}</span>
		{/if}
	</div>
	{#if _av_show}
