...
try {
  if (typeof window !== "undefined") {
    ...
    window.__DASH_PICKERS__.iconSelect = (v) => {
      try {
        if (iconSelectCallback) iconSelectCallback(v);
      } catch (e) {
...
async function switchTab(tabName) {
  ...
  if (tabName === "uploads")
    populateUploadsGrid(uploadSearch ? uploadSearch.value : "");
...
async function populateUploadsGrid(filter) {
  ...
  if (!userUploadsCache) {
    uploadsGrid.innerHTML = Array(12)
      .fill(0)
      .map(
        () => `
  ...
  if (!filtered.length && term) {
    uploadsGrid.innerHTML =
      '<div class="col-span-full text-center text-slate-500 py-12">Aucun résultat pour "' +
      filter +
      '".</div>';
    return;
  }
  ...
  filtered.forEach((u) => {
    ...
    btn.type = "button";
    btn.className =
      "p-3 rounded-2xl border border-slate-800 bg-slate-950/40 hover:bg-slate-800 hover:border-violet-500/50 hover:shadow-lg hover:shadow-violet-500/5 transition-all flex flex-col items-center justify-center gap-3 group relative";

    const imgContainer = document.createElement("div");
    imgContainer.className =
      "size-20 rounded-xl bg-slate-800/50 animate-pulse shrink-0 flex items-center justify-center overflow-hidden border border-slate-800/50";
    ...
    span.className =
      "text-[11px] font-medium text-slate-400 group-hover:text-slate-200 truncate max-w-full block text-center transition-colors px-1";
...
export function openIconModal(onSelect, targetId) {
  ...
  if (uploadSearch)
    uploadSearch.oninput = () => populateUploadsGrid(uploadSearch.value);
  ...
  if (triggerUploadBtn) {
    triggerUploadBtn.onclick = () => {
      ...
      // Specific overrides
      if (currentTargetField === "linkModalIconInput")
        inputId = "linkModalIconUpload";
...
export async function populateIconGrid(filterText) {
  ...
  items.forEach((i) => {
    const key =
      (i.displayName && String(i.displayName).charAt(0).toUpperCase()) || "#";
  ...
  Object.keys(groups)
    ...
    .forEach((letter) => {
      ...
      header.className =
        "text-xs font-bold text-slate-500 uppercase tracking-wider col-span-full mt-4 mb-2 pl-1";
      ...
      groups[letter].forEach((i) => {
        ...
        card.className =
          "p-3 rounded-xl border border-slate-800 bg-slate-950 hover:bg-slate-800 hover:border-violet-500/50 transition-all flex flex-col items-center justify-center gap-3";

        const imgContainer = document.createElement("div");
        imgContainer.className =
          "h-10 w-10 rounded-lg bg-slate-800 animate-pulse shrink-0 flex items-center justify-center overflow-hidden";
        ...
        label.className =
          "text-xs text-slate-400 group-hover:text-white text-center truncate max-w-full";
...
export function pickerSelect(index) {
  try {
    pickerOnSelect && pickerOnSelect(index);
  } finally {
    closePicker();
  }
}
...
function defaultRenderCard(item, idx) {
  ...
  card.setAttribute("type", "button");
  card.className =
    "p-3 rounded border border-slate-800 bg-slate-900 hover:bg-slate-800 text-left";
  ...
  if (pickerType === "canvas")
    small.textContent = item?.animationName
      ? `${item.animationName} (index ${idx})`
      : `Index ${idx}`;
  if (pickerType === "anim")
...
export function renderPickerGrid(query) {
  ...
  pickerData.forEach((item, idx) => {
    ...
    const card = pickerRenderCard
      ? pickerRenderCard(item, idx)
      : defaultRenderCard(item, idx);
...
export function renderBtnThemeCard(item, idx) {
  ...
  const imgContainer = document.createElement("div");
  imgContainer.className =
    "h-8 w-8 rounded bg-slate-800 animate-pulse shrink-0 flex items-center justify-center overflow-hidden";
  ...
  card.addEventListener("click", () => {
    if (pickerOnSelect) pickerOnSelect(idx);
    closePicker();
  });
  return card;
}
...
export function ensurePlatformEntryModal() {
  ...
  panel.className =
    "mx-4 rounded border border-slate-800 bg-slate-900 shadow-lg overflow-hidden";
  const header = document.createElement("div");
  header.className =
    "flex items-center justify-between px-4 py-2 border-b border-slate-800";
  ...
  closeBtn.className =
    "h-8 w-8 inline-flex items-center justify-center rounded bg-slate-800 border border-slate-700 hover:bg-slate-700";
  closeBtn.setAttribute("aria-label", "Fermer");
  closeBtn.innerHTML =
    '<svg class="h-4 w-4 text-slate-200" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>';
  ...
  fieldHandle.className =
    "w-full px-3 py-2 rounded bg-slate-800 border border-slate-700 text-sm";
  const fieldCountry = document.createElement("input");
  fieldCountry.type = "text";
  fieldCountry.placeholder = "Code pays (ex: fr, us)";
  fieldCountry.className =
    "w-full px-3 py-2 rounded bg-slate-800 border border-slate-700 text-sm";
  ...
  btnCancel.className =
    "h-9 px-3 rounded bg-slate-800 border border-slate-700 hover:bg-slate-700 text-sm";
  btnCancel.textContent = "Annuler";
  const btnOk = document.createElement("button");
  btnOk.type = "button";
  btnOk.className =
    "h-9 px-3 rounded bg-emerald-600 hover:bg-emerald-500 text-sm text-white";
...
try {
  window.__OPEN_PLATFORM_MODAL__ = openPlatformEntryModal;
} catch (e) {
  console.error("Caught error", e);
}
