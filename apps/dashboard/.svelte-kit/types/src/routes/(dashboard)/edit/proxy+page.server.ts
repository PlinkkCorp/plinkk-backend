...
export const actions = {
  update: async ({ request, locals, url }: import('./$types').RequestEvent) => {
    const user = locals.user;
  ...
  saveLink: async ({ request, locals, url }: import('./$types').RequestEvent) => {
    const user = locals.user;
    if (!user) return { success: false, error: 'Unauthorized' };
  ...
  deleteLink: async ({ request, locals }: import('./$types').RequestEvent) => {
    const user = locals.user;
    if (!user) return { success: false, error: 'Unauthorized' };
  ...
  saveSocialIcon: async ({ request, locals, url }: import('./$types').RequestEvent) => {
    const user = locals.user;
    if (!user) return { success: false, error: 'Unauthorized' };
  ...
  deleteSocialIcon: async ({ request, locals }: import('./$types').RequestEvent) => {
    const user = locals.user;
    if (!user) return { success: false, error: 'Unauthorized' };
