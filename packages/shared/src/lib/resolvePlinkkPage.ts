...
export function parseIdentifier(id?: string | null): {
  kind: "default" | "index" | "slug";
  value?: number | string;
} {
...
export async function resolvePlinkkPage(
  prisma: PrismaClient,
  username: string,
  ...
  let page = await prisma.plinkk.findFirst({ where: { slug: username } });

  if (!user && !page) return { status: 404 as const, error: "user_not_found" };

  if (user && !page) {
    page = await (async () => {
      if (parsed.kind === "default") {
        const byDefault = await prisma.plinkk.findFirst({
          where: { userId: user.id, isDefault: true },
        });
        if (byDefault) return byDefault;
        return prisma.plinkk.findFirst({
          where: { userId: user.id, index: 0 },
        });
      }
      if (parsed.kind === "index") {
        return prisma.plinkk.findFirst({
          where: { userId: user.id, index: parsed.value as number },
        });
      }
      return prisma.plinkk.findFirst({
        where: { userId: user.id, slug: parsed.value as string },
      });
    })();
  }
