...
export async function populateIconGrid(filterText) {
  ...
  iconCatalog
    ...
    .forEach((i) => {
      ...
      card.className =
        "p-3 rounded border border-slate-800 bg-slate-900 hover:bg-slate-800 flex flex-col items-center gap-2";
      const img = document.createElement("img");
      img.src = `https://cdn.plinkk.fr/icons/${i.slug}.svg`;
...
export function renderBtnThemeCard(item, idx) {
  ...
  img.alt = item.name || "";
  img.className =
    "h-8 w-8 object-contain rounded bg-slate-800 border border-slate-700";
