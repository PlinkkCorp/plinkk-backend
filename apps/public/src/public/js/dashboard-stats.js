...
function buildBarChart(container, data, { maxBars = 10 } = {}) {
  ...
  if (!data.length) {
    ...
    div.textContent = 'Aucune donnée à afficher.';
...
function drawLineChart(container, series) {
  ...
  area.setAttribute(
    "d",
    `${d} L ${pad.l + innerW} ${pad.t + innerH} L ${pad.l} ${pad.t + innerH} Z`,
  );
  ...
  const tests = [
    {
      ...
      re: /(twitter|x\.com|instagram|facebook|tiktok|linkedin|threads|bluesky)/,
    },
    { name: "Vidéo", re: /(youtube|youtu\.be|vimeo|twitch|dailymotion)/ },
    {
      name: "Musique",
      re: /(spotify|soundcloud|music\.apple|deezer|bandcamp)/,
    },
    { name: "Code", re: /(github|gitlab|bitbucket|npmjs|pypi|docker\.com)/ },
    {
      name: "Articles & Blog",
      re: /(medium\.com|dev\.to|hashnode|substack|notion\.site|ghost)/,
    },
    {
      name: "E‑commerce",
      re: /(gumroad|shopify|buymeacoffee|patreon|ko-fi|paypal)/,
    },
  ];
...
function aggregateCategories(rows) {
  ...
  const list = Array.from(map.entries()).map(([label, clicks]) => ({
    label,
    clicks,
  }));
...
function updateKPIs({ views, links }) {
  ...
  const elViews = qs("#kpiViews");
  if (elViews) elViews.textContent = formatNumber(views || 0);
  const elClicks = qs("#kpiClicks");
  if (elClicks) elClicks.textContent = formatNumber(totalClicks);
  const elAvg = qs("#kpiAvg");
  if (elAvg) elAvg.textContent = formatNumber(Math.round(avgClicks));
  const elTop = qs("#kpiTop");
  if (elTop)
    elTop.textContent = top
      ? `${top.text || top.name || "Sans titre"} (${formatNumber(top.clicks || 0)})`
      : "—";
}
...
function renderTable(rows) {
  ...
  rows.forEach((r) => {
    ...
    td1.className = 'px-3 py-2 text-xs text-slate-400';
    td1.textContent = r.id;
    const td2 = document.createElement('td');
    td2.className = 'px-3 py-2';
    td2.textContent = r.label;
    const td3 = document.createElement('td');
    td3.className = 'px-3 py-2 text-indigo-300 truncate';
    const a = document.createElement('a');
    ...
    td4.className = 'px-3 py-2 text-right';
    td4.textContent = formatNumber(r.clicks);
    tr.appendChild(td1);
    tr.appendChild(td2);
    tr.appendChild(td3);
    tr.appendChild(td4);
...
async function fetchDailyViews(days) {
  ...
  const res = await fetch(`/dashboard/stats/views?from=${from}&to=${to}`, {
...
async function main() {
  ...
  if (root) {
    ...
    try {
      data.links = JSON.parse(linksRaw) || [];
    } catch {
      data.links = [];
    }
    data.views = views;
  ...
  const apply = () => {
    ...
    qs("#exportCsv").onclick = () =>
      downloadBlob(makeCSV(rows), "stats-links.csv");
  };
  ["#fSearch", "#fMin", "#fTop"].forEach((sel) =>
    qs(sel)?.addEventListener("input", apply),
  );
  apply();
  ...
  async function refreshViews() {
    ...
    try {
      ...
      div.textContent = 'Erreur de chargement des vues datées' + statusMsg + details;
  ...
  async function fetchDailyClicks(days) {
    ...
    const fromDate = new Date(now.getTime() - (days - 1) * 86400000);
    const from = fromDate.toISOString().slice(0, 10);
    const res = await fetch(`/dashboard/stats/clicks?from=${from}&to=${to}`, {
      credentials: "same-origin",
    });
    if (!res.ok) {
      let msg = "";
  ...
  async function refreshClicks() {
    ...
    try {
      ...
      const statusMsg = e && e.status === 401 ? " (non connecté)" : "";
      const details = e && e.message ? ` — ${e.message}` : "";
      chartClicks.innerHTML = '';
      const div = document.createElement('div');
      div.className = 'text-xs text-rose-400';
      div.textContent = 'Erreur de chargement des clics datés' + statusMsg + details;
      chartClicks.appendChild(div);
    }
  }
