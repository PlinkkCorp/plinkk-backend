...
export function enableDragHandle(
  handleEl,
  rowEl,
  containerEl,
  itemsArray,
  ...
  const isDraggableRow = (el) =>
    !!(
      el &&
      el.dataset &&
      el.dataset.dragIndex !== undefined &&
      el.dataset.dragIndex !== null
    );
  ...
  function onPointerMove(e) {
    ...
    const rows = Array.from(containerEl.children).filter(
      (c) => c !== ghost && c !== placeholder && isDraggableRow(c),
    );
  ...
  function onPointerUp(e) {
    ...
    if (
      originalIdx >= 0 &&
      originalIdx < itemsArray.length &&
      desiredIndex !== originalIdx
    ) {
