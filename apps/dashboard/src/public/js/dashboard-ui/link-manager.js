...
export class LinkManager {
  ...
  updateCategoryDropdown() {
    ...
    const cfg =
      (window.__PLINKK_GET_CONFIG__ && window.__PLINKK_GET_CONFIG__()) ||
  ...
  init() {
    ...
    if (this.schemeBtn && this.schemeLabel) {
      this.schemeBtn.addEventListener("click", () => {
        this.currentScheme =
          this.currentScheme === "https://" ? "http://" : "https://";
    ...
    if (addLinkBtn)
      addLinkBtn.addEventListener("click", () => this.openModal("LINK"));

    const addHeaderBtn = document.getElementById("addHeader");
    if (addHeaderBtn)
      addHeaderBtn.addEventListener("click", () => this.openModal("HEADER"));
    ...
    if (this.inputs.iconUpload) {
      this.inputs.iconUpload.addEventListener("change", async (e) => {
        ...
        if (this.inputs.iconInput)
          this.inputs.iconInput.value = "Chargement...";
        ...
        try {
          ...
          if (data.ok && data.url) {
            if (this.inputs.iconInputUrl)
              this.inputs.iconInputUrl.value = data.url;
            ...
            if (this.inputs.iconInput)
              this.inputs.iconInput.value = originalText;
          }
        } catch (err) {
    ...
    if (pickIconBtn) {
      pickIconBtn.onclick = () => {
        const openIconModal =
          window.openIconModal || window.__DASH_PICKERS__?.openIconModal;
        if (openIconModal)
          openIconModal((icon) => {
            if (this.inputs.iconInput) {
              this.inputs.iconInput.value = icon;
              this.inputs.iconInput.dispatchEvent(
    ...
    window.__PLINKK_SYNC_SIDEBAR__ = () => {
      ...
      const currentLinks =
        window.__PLINKK_GET_CONFIG__?.()?.links ||
  ...
  async initThemePicker() {
    ...
    try {
      ...
      const { btnIconThemeConfig } =
        await import("../../config/btnIconThemeConfig.js");
      ...
      btnIconThemeConfig.forEach((theme) => {
        ...
        btn.className =
          "theme-card group relative flex flex-col items-center gap-2 p-2 rounded-lg border border-slate-800 bg-slate-800/30 hover:bg-slate-800 transition-all ring-2 ring-transparent data-[active=true]:ring-violet-500 data-[active=true]:bg-violet-500/10";
      ...
      const systemBtn = this.themeContainer.querySelector(
  ...
  setButtonTheme(themeClass) {
    ...
    if (this.themeContainer) {
      this.themeContainer
        ...
        .forEach((btn) => {
          btn.dataset.active = (
            btn.dataset.themeValue === themeClass
          ).toString();
        });
    }
  }
  ...
  openModal(type = "LINK", existingData = null) {
    ...
    this.modalTitleHeader.textContent = existingData
      ? "Modifier l'élément"
      : "Ajouter un élément";
    ...
    switch (type) {
      ...
      case "FORM":
        ...
        if (
          ...
          this.inputs.iconInput &&
          !this.inputs.iconInput.value
        ) {
  ...
  populateForm(data) {
    ...
    if (data.formData) {
      ...
      if (i.formSuccessMsg)
        i.formSuccessMsg.value = data.formData.successMessage || "";
    }
  }
  ...
  async saveLink() {
    ...
    const currentConfig = window.__PLINKK_GET_CONFIG__
      ? window.__PLINKK_GET_CONFIG__()
      : null;
    let links =
      currentConfig && currentConfig.links ? [...currentConfig.links] : [];
    ...
    const data = {
      ...
      icon: finalIcon,
      index: editingId
        ? links.find((l) => l.id === editingId)?.index || 0
        : links.length > 0
          ? Math.max(...links.map((l) => l.index || 0)) + 1
          : 0,
      ...
      formData:
        type === "FORM"
          ? {
              ...
              successMessage:
                this.inputs.formSuccessMsg?.value ||
                "Message envoyé avec succès !",
              fields: (editingId
                ? links.find((l) => l.id === editingId)?.formData?.fields
                : null) || [
                {
                  label: "Nom",
                  type: "text",
    ...
    try {
      ...
      if (result && result.links) {
        if (data.id.startsWith("new_")) {
          const newLink = result.links.find(
            (l) => l.text === data.text && l.url === data.url,
          );
        ...
        if (window.__INITIAL_STATE__)
          window.__INITIAL_STATE__.links = result.links;
  ...
  async saveNewOrder() {
    ...
    const ids = Array.from(list.querySelectorAll(".group[data-id]")).map(
      (el) => el.dataset.id,
    );
    const currentConfig = window.__PLINKK_GET_CONFIG__
      ? window.__PLINKK_GET_CONFIG__()
    ...
    if (saveFn) {
      try {
        ...
        if (result && result.links) {
          ...
          const fullConfig = {
            ...window.__PLINKK_GET_CONFIG__(),
            links: result?.links || sortedLinks,
          };
          syncConfigToIframe(fullConfig);
        }
      } catch (e) {
  ...
  renderLinksList(links) {
    ...
    if (!links || links.length === 0) {
      list.innerHTML =
        '<div class="text-center py-8 text-slate-500 text-sm italic">Aucun lien créé.</div>';
      return;
    }
  ...
  getLinkItemHtml(link) {
    ...
    switch (type) {
      case "HEADER":
        typeBadge =
          '<span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-slate-800 text-slate-400 uppercase tracking-wider">Titre</span>';
        break;
      case "EMBED":
        typeBadge =
          '<span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-indigo-500/20 text-indigo-400 uppercase tracking-wider">Embed</span>';
        break;
      case "MUSIC":
        typeBadge =
          '<span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-pink-500/20 text-pink-400 uppercase tracking-wider">Musique</span>';
        break;
      case "FORM":
        typeBadge =
          '<span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-500/20 text-emerald-400 uppercase tracking-wider">Formulaire</span>';
        break;
      default:
        typeBadge =
          '<span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-blue-500/20 text-blue-400 uppercase tracking-wider">Lien</span>';
        break;
    }
    const themeBadge =
