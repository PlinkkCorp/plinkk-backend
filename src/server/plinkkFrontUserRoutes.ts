    ...
    async function (request, reply) {
      ...
      try {
        ...
        if (pageBySlug) {
          ...
          if (resolved.status === 200) {
            ...
            try {
              const u = await prisma.user.findUnique({ where: { id: resolved.user.id }, select: { email: true } });
              if (u?.email) {
                const ban = await prisma.bannedEmail.findFirst({ where: { email: u.email, revoquedAt: null } });
                if (ban) {
                  const isActive =
                    ban.time == null || ban.time < 0 || (new Date(ban.createdAt).getTime() + ban.time * 60000) > Date.now();
                  if (isActive) {
                    const until = (typeof ban.time === "number" && ban.time > 0)
                      ? new Date(new Date(ban.createdAt).getTime() + ban.time * 60000).toISOString()
                      : null;
                    return reply.view("erreurs/banned.ejs", { reason: ban.reason || "Violation des règles", email: u.email, until });
                  }
                }
              }
            } catch (e) {}
      ...
      try {
        ...
        if (u?.email) {
          const ban = await prisma.bannedEmail.findFirst({ where: { email: u.email, revoquedAt: null } });
          if (ban) {
            const isActive = ban.time == null || ban.time < 0 || (new Date(ban.createdAt).getTime() + ban.time * 60000) > Date.now();
            if (isActive) {
              const until = (typeof ban.time === "number" && ban.time > 0)
                ? new Date(new Date(ban.createdAt).getTime() + ban.time * 60000).toISOString()
                : null;
