...
export function ensureCanvasPreviewModal() {
  ...
  document.addEventListener("keydown", (e) => {
    if (
      e.key === "Escape" &&
      canvasPreviewModal &&
      !canvasPreviewModal.classList.contains("hidden")
    )
      close();
  });
...
export function refreshSelectedCanvasPreview({
  cfg,
  canvaEnableEl,
  selectedCanvasIndexEl,
  ...
  const idx = Math.max(
    0,
    Math.min(Number(selectedCanvasIndexEl?.value || 0), canvases.length - 1),
  );
  const item = canvases[idx];
  const enabled = !!canvaEnableEl?.checked;
  if (selectedCanvasPreviewOverlay)
    selectedCanvasPreviewOverlay.classList.toggle("hidden", enabled);
...
export function renderCanvasCard(item, idx, onSelect) {
  ...
  previewBtn.className =
    "h-8 px-3 rounded bg-slate-800 border border-slate-700 hover:bg-slate-700 text-sm";
