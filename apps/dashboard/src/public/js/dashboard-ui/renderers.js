...
export function renderSkeletons(container, count = 3) {
  ...
  for (let i = 0; i < count; i++) {
    const row = el("div", {
      class:
        "flex items-center justify-between gap-3 p-3 rounded-xl border border-slate-800 bg-slate-900 animate-pulse",
    });
    const left = el("div", { class: "flex items-center gap-3 min-w-0 flex-1" });
    const grip = el("div", {
      class: "h-8 w-8 rounded bg-slate-800 border border-slate-700",
    });
    const icon = el("div", {
      class: "h-10 w-10 rounded bg-slate-800 border border-slate-700",
    });
...
export function emptyState({ title, description, actionLabel, onAction }) {
  const box = el("div", {
    class:
      "rounded border border-dashed border-slate-700 bg-slate-900/40 p-4 text-center text-sm text-slate-300",
  });
  if (title) box.append(el("div", { class: "font-medium mb-1", text: title }));
  if (description)
    box.append(
      el("div", { class: "text-xs text-slate-400 mb-3", text: description }),
    );
  if (actionLabel && onAction) {
    const btn = el("button", {
      class:
        "h-10 inline-flex items-center gap-2 px-3 rounded bg-slate-800 hover:bg-slate-700 border border-slate-700 text-sm",
      text: actionLabel,
      type: "button",
    });
  ...
  if (!Array.isArray(colors) || colors.length === 0) {
    container.append(
      emptyState({
        ...
        description:
          "Ajoutez au moins une couleur pour créer le dégradé d’arrière‑plan.",
        actionLabel: "+ Ajouter une couleur",
        onAction: () => {
          colors.push("#ffffff");
          renderBackground({ container, addBtn, colors, scheduleAutoSave });
          scheduleAutoSave();
        },
      }),
    );
    ...
    colors.forEach((c, idx) => {
      ...
      const gripBtn = el("button", {
        type: "button",
        class:
          "h-9 w-9 inline-flex items-center justify-center rounded bg-slate-800 border border-slate-700 hover:bg-slate-700 mr-1",
        title: "Déplacer",
        "aria-label": "Déplacer",
      });
      ...
      const color = el("input", {
        ...
        value: c,
        class:
          "h-10 w-full rounded bg-slate-900 border border-slate-800 p-1 flex-1",
      });
      const rm = trashButton(() => {
        colors.splice(idx, 1);
        renderBackground({ container, addBtn, colors, scheduleAutoSave });
        scheduleAutoSave();
      });
      color.addEventListener("input", () => {
        colors[idx] = color.value;
        scheduleAutoSave();
      });
      const rmWrap = el("div", { class: "w-10 flex justify-end" });
      rmWrap.append(rm);
...
export function renderNeon({
  ...
  if (addBtn)
    addBtn.onclick = () => {
      colors.push("#7289DA");
      renderNeon({ container, addBtn, colors, neonEnableEl, scheduleAutoSave });
      scheduleAutoSave();
    };
  ...
  if (neonEnableEl) {
    ...
    neonEnableEl.title = hasColors
      ? ""
      : "Ajoutez au moins une couleur pour activer le néon";
...
export function renderLabels({ container, addBtn, labels, scheduleAutoSave }) {
  ...
  if (!Array.isArray(labels) || labels.length === 0) {
    container.append(
      emptyState({
        title: "Aucun label",
        description:
          "Ajoutez des badges pour mettre en valeur vos informations.",
        ...
        onAction: () => {
          ...
          renderLabels({ container, addBtn, labels, scheduleAutoSave });
          scheduleAutoSave();
        },
      }),
    );
    ...
    labels.forEach((l, idx) => {
      ...
      const gripBtn = el("button", {
        type: "button",
        class:
          "h-9 w-9 inline-flex items-center justify-center rounded bg-slate-800 border border-slate-700 hover:bg-slate-700 mr-3",
        title: "Déplacer",
        "aria-label": "Déplacer",
      });
      gripBtn.style.cursor = "grab";
      ...
      const data = el("input", {
        type: "text",
        value: l.data || "",
        class: "px-3 py-2 rounded bg-slate-900 border border-slate-800 flex-1",
      });
      const dataWrapper = el("div", {
        class: "flex items-center gap-2 w-full md:w-1/3",
      });
      ...
      const color = el("input", {
        ...
        class:
          "h-10 w-full md:w-1/6 rounded bg-slate-900 border border-slate-800 p-1",
      });
      const fontColor = el("input", {
        type: "color",
      ...
      const pickBtn = el("button", {
        ...
        text: "Choisir",
        class:
          "h-9 px-3 inline-flex items-center justify-center rounded bg-slate-800 border border-slate-700 hover:bg-slate-700 text-sm",
      });
      const rm = trashButton(() => {
        labels.splice(idx, 1);
        renderLabels({ container, addBtn, labels, scheduleAutoSave });
      ...
      function openLabelPresetPicker() {
        ...
        openPicker({
          ...
          renderCard: (item, i) => {
            ...
            card.type = "button";
            card.className =
              "p-3 rounded border border-slate-800 bg-slate-900 hover:bg-slate-800 text-left flex items-center gap-3";
            const swatch = document.createElement("span");
            swatch.className =
              "inline-block h-6 w-6 rounded border border-slate-700";
            ...
            card.addEventListener("click", () => {
              window.__DASH_PICKERS__.pickerOnSelect?.(i);
            });
            return card;
          },
          onSelect: (i) => {
      ...
      fontColor.addEventListener("input", () => {
        l.fontColor = fontColor.value;
        scheduleAutoSave();
      });

      const pickWrap = el("div", {
        class: "w-full md:w-1/6 flex justify-center md:justify-start",
      });
  ...
  if (addBtn)
    addBtn.onclick = () => {
      labels.push({ data: "Nouveau", color: "#FF6384", fontColor: "#FFFFFF" });
      renderLabels({ container, addBtn, labels, scheduleAutoSave });
      scheduleAutoSave();
    };
}
...
export function renderSocial({ container, addBtn, socials, scheduleAutoSave }) {
  ...
  const SOCIAL_PLATFORMS = [
    ...
    },
    {
      ...
      iconSlug: "youtube-alt",
    },
    {
      id: "twitch",
      name: "Twitch",
      pattern: "https://twitch.tv/{handle}",
      iconSlug: "twitch",
    },
    {
      id: "instagram",
      name: "Instagram",
      pattern: "https://instagram.com/{handle}",
      iconSlug: "instagram",
    },
    {
      id: "facebook",
      name: "Facebook",
      pattern: "https://facebook.com/{handle}",
      iconSlug: "facebook",
    },
    {
      id: "linkedin",
      name: "LinkedIn",
      pattern: "https://www.linkedin.com/in/{handle}",
      iconSlug: "linkedin",
    },
    {
      id: "discord",
      name: "Discord Server",
      pattern: "https://discord.gg/{handle}",
      iconSlug: "discord",
    },
    {
      id: "apple-music",
      name: "Apple Music",
      pattern: "https://music.apple.com/{country}/{path}",
      iconSlug: "apple-music-alt",
    },
    {
      id: "apple-podcasts",
      name: "Apple Podcasts",
      pattern: "https://podcasts.apple.com/{country}/{path}",
      iconSlug: "apple-podcasts-alt",
    },
    {
      id: "tiktok",
      name: "TikTok",
    ...
    },
    {
      id: "spotify",
      name: "Spotify",
      pattern: "https://open.spotify.com/user/{handle}",
      iconSlug: "spotify",
    },
    {
      id: "snapchat",
      name: "Snapchat",
      pattern: "https://snapchat.com/add/{handle}",
      iconSlug: "snapchat",
    },
    {
      id: "reddit",
      name: "Reddit",
      pattern: "https://reddit.com/user/{handle}",
      iconSlug: "reddit",
    },
    {
      id: "pinterest",
      name: "Pinterest",
      pattern: "https://pinterest.com/{handle}",
      iconSlug: "pinterest",
    },
    {
      id: "whatsapp",
      name: "WhatsApp",
      pattern: "https://wa.me/{handle}",
      iconSlug: "whatsapp",
    },
    {
      id: "telegram",
      name: "Telegram",
      pattern: "https://t.me/{handle}",
      iconSlug: "telegram",
    },
    {
      id: "threads",
    ...
    },
    {
      id: "signal",
      name: "Signal",
      pattern: "https://signal.me/#p/{handle}",
      iconSlug: "signal",
    },
    {
      id: "messenger",
      name: "Messenger",
      pattern: "https://m.me/{handle}",
      iconSlug: "messenger",
    },
    {
      id: "mastodon",
    ...
    },
    {
      id: "soundcloud",
      name: "SoundCloud",
      pattern: "https://soundcloud.com/{handle}",
      iconSlug: "soundcloud",
    },
    {
      id: "bandcamp",
      name: "Bandcamp",
      pattern: "https://{handle}.bandcamp.com",
      iconSlug: "bandcamp",
    },
    {
      id: "dribbble",
      name: "Dribbble",
      pattern: "https://dribbble.com/{handle}",
      iconSlug: "dribbble",
    },
    {
      id: "behance",
      name: "Behance",
      pattern: "https://behance.net/{handle}",
      iconSlug: "behance",
    },
    {
      id: "figma",
      name: "Figma",
      pattern: "https://figma.com/@{handle}",
      iconSlug: "figma",
    },
    {
      id: "gitlab",
      name: "GitLab",
      pattern: "https://gitlab.com/{handle}",
      iconSlug: "gitlab",
    },
    {
      id: "medium",
      name: "Medium",
      pattern: "https://medium.com/@{handle}",
      iconSlug: "medium",
    },
    {
      id: "substack",
      name: "Substack",
      pattern: "https://{handle}.substack.com",
      iconSlug: "substack",
    },
    {
      id: "patreon",
      name: "Patreon",
      pattern: "https://patreon.com/{handle}",
      iconSlug: "patreon",
    },
    {
      id: "ko-fi",
      name: "Ko-fi",
      pattern: "https://ko-fi.com/{handle}",
      iconSlug: "ko-fi",
    },
    {
      id: "buy-me-a-coffee",
      name: "Buy Me a Coffee",
      pattern: "https://buymeacoffee.com/{handle}",
      iconSlug: "buy-me-a-coffee",
    },
    {
      id: "paypal",
      name: "PayPal",
      pattern: "https://paypal.me/{handle}",
      iconSlug: "paypal",
    },
    {
      id: "venmo",
      name: "Venmo",
      pattern: "https://venmo.com/{handle}",
      iconSlug: "venmo",
    },
    {
      id: "kick",
      name: "Kick",
      pattern: "https://kick.com/{handle}",
      iconSlug: "kick",
    },
    {
      id: "vimeo",
      name: "Vimeo",
      pattern: "https://vimeo.com/{handle}",
      iconSlug: "vimeo",
    },
    {
      id: "tumblr",
      name: "Tumblr",
      pattern: "https://{handle}.tumblr.com",
      iconSlug: "tumblr",
    },
    {
      id: "slack",
      name: "Slack",
      pattern: "https://{handle}.slack.com",
      iconSlug: "slack",
    },
    {
      id: "notion",
      name: "Notion",
      pattern: "https://notion.so/{handle}",
      iconSlug: "notion",
    },
    {
      id: "trello",
      name: "Trello",
      pattern: "https://trello.com/{handle}",
      iconSlug: "trello",
    },
    {
      id: "steam",
      name: "Steam",
      pattern: "https://steamcommunity.com/id/{handle}",
      iconSlug: "steam",
    },
    {
      id: "onlyfans",
      name: "OnlyFans",
      pattern: "https://onlyfans.com/{handle}",
      iconSlug: "onlyfans",
    },
    {
      id: "etsy",
      name: "Etsy",
      pattern: "https://etsy.com/shop/{handle}",
      iconSlug: "etsy",
    },
    {
      id: "fiverr",
      name: "Fiverr",
      pattern: "https://fiverr.com/{handle}",
      iconSlug: "fiverr",
    },
    {
      id: "calendly",
      name: "Calendly",
      pattern: "https://calendly.com/{handle}",
      iconSlug: "calendly",
    },
    {
      id: "zoom",
      name: "Zoom",
      pattern: "https://zoom.us/j/{handle}",
      iconSlug: "zoom",
    },
    {
      id: "amazon",
      name: "Amazon",
      pattern: "https://amazon.com/shop/{handle}",
      iconSlug: "amazon",
    },
    {
      id: "goodreads",
      name: "Goodreads",
      pattern: "https://goodreads.com/{handle}",
      iconSlug: "goodreads",
    },
    {
      id: "letterboxd",
      name: "Letterboxd",
      pattern: "https://letterboxd.com/{handle}",
      iconSlug: "letterboxd",
    },
    {
      id: "strava",
      name: "Strava",
      pattern: "https://strava.com/athletes/{handle}",
      iconSlug: "strava",
    },
    {
      id: "unsplash",
      name: "Unsplash",
      pattern: "https://unsplash.com/@{handle}",
      iconSlug: "unsplash",
    },
    {
      id: "flickr",
      name: "Flickr",
      pattern: "https://flickr.com/photos/{handle}",
      iconSlug: "flickr",
    },
    {
      id: "vsco",
      name: "VSCO",
      pattern: "https://vsco.co/{handle}",
      iconSlug: "vsco",
    },
    {
      id: "artstation",
      name: "ArtStation",
      pattern: "https://artstation.com/{handle}",
      iconSlug: "artstation",
    },
    {
      id: "devto",
      name: "DEV.to",
      pattern: "https://dev.to/{handle}",
      iconSlug: "dev-to",
    },
    {
      id: "hashnode",
      name: "Hashnode",
      pattern: "https://hashnode.com/@{handle}",
      iconSlug: "hashnode",
    },
    {
      id: "stackoverflow",
      name: "Stack Overflow",
      pattern: "https://stackoverflow.com/users/{handle}",
      iconSlug: "stack-overflow",
    },
    {
      id: "producthunt",
      name: "Product Hunt",
      pattern: "https://producthunt.com/@{handle}",
      iconSlug: "product-hunt",
    },
    {
      id: "vrchat",
      name: "VRChat",
      pattern: "https://vrchat.com/home/user/{handle}",
      iconSlug: "vrchat",
    },
    {
      id: "line",
      name: "LINE",
      pattern: "https://line.me/ti/p/{handle}",
      iconSlug: "line",
    },
  ];
  try {
  ...
  if (!Array.isArray(socials) || socials.length === 0) {
    container.append(
      emptyState({
        ...
        actionLabel: "+ Ajouter",
        onAction: () => {
          socials.push({ url: "https://", icon: "github" });
    ...
    socials.forEach((s, idx) => {
      ...
      const row = el("div", {
        class:
          "flex items-center justify-between gap-3 p-3 rounded-xl border border-slate-800 bg-slate-900",
      });
      row.dataset.dragIndex = String(idx);
      ...
      const grip = el("button", {
        type: "button",
        class:
          "h-8 w-8 inline-flex items-center justify-center rounded bg-slate-800 border border-slate-700 hover:bg-slate-700 text-slate-400 cursor-grab",
        title: "Déplacer",
      });
      ...
      const iconContainer = el("div", {
        class:
          "h-10 w-10 rounded bg-slate-800 animate-pulse shrink-0 overflow-hidden border border-slate-700 flex items-center justify-center",
      });
      let srcUrl = "";
      if (s.icon) {
        srcUrl = isUrlish(s.icon)
          ? s.icon
          : `https://cdn.plinkk.fr/icons/${s.icon}.svg`;
      }
      ...
      title.textContent =
        s.name || s.title || (s.url || "").replace(/^https?:\/\//, "");
      const small = el("div", { class: "text-xs text-slate-400 truncate" });
      ...
      const editBtn = el("button", {
        type: "button",
        class:
          "px-3 py-1 rounded text-xs bg-slate-800 hover:bg-slate-700 text-slate-200",
        title: "Modifier",
      });
      ...
      function openEditor() {
        const editor = el("div", {
          class:
            "flex flex-col md:flex-row items-center gap-2 p-2 bg-slate-950 rounded border border-slate-800",
        });
        const gripBtn = el("button", {
          type: "button",
          class:
            "h-9 w-9 inline-flex items-center justify-center rounded bg-slate-800 border border-slate-700 hover:bg-slate-700 ml-0 self-center shrink-0",
          title: "Déplacer",
        });
        gripBtn.style.cursor = "grab";
        gripBtn.appendChild(createGripSVG(18));
        const urlSourceSel = el("select", {
          class:
            "h-10 px-2 rounded bg-slate-900 border border-slate-800 text-sm w-full md:w-40",
        });
        ["custom"].forEach((v) =>
          urlSourceSel.appendChild(
            el("option", { value: v, text: "Personnalisé" }),
          ),
        );
        ...
        const url = el("input", {
          ...
          value: s.url || "",
          class:
            "pl-3 pr-20 py-2 w-full rounded bg-slate-900 border border-slate-800",
        });
        const urlPickBtn = el("button", {
          type: "button",
          text: "Choisir",
          class:
            "absolute right-1 top-1 h-8 px-2 text-xs rounded bg-slate-800 border border-slate-700 hover:bg-slate-700",
        });
        ...
        const sourceSel = el(
          ...
          {
            class:
              "h-10 px-2 rounded bg-slate-900 border border-slate-800 text-sm",
          },
          [],
        );
        ...
        const iconName = el("input", {
          ...
          value: s.icon || "",
          class:
            "pl-3 pr-20 py-2 w-full rounded bg-slate-900 border border-slate-800 cursor-pointer",
        ...
        const iconUrlInput = el("input", {
          type: "url",
          class:
            "px-3 py-2 w-full rounded bg-slate-900 border border-slate-800",
        ...
        const fileInput = el("input", {
          ...
          class:
            "block w-full text-sm text-slate-300 file:mr-2 file:py-2 file:px-3 file:rounded file:border-0 file:text-sm file:bg-slate-800 file:text-slate-200 hover:file:bg-slate-700",
        });
        iconUploadWrap.append(fileInput);
        const saveBtn = el("button", {
          type: "button",
          class: "px-3 py-1 rounded text-xs bg-emerald-700 text-white",
        });
        saveBtn.textContent = "Valider";
        const cancelBtn = el("button", {
          type: "button",
          class: "px-3 py-1 rounded text-xs bg-slate-800 text-slate-200",
        });
        cancelBtn.textContent = "Annuler";

        function setPreviewByValue(val) {
          if (isUrlish(val)) iconPreview.src = val;
          else iconPreview.src = `https://cdn.plinkk.fr/icons/${val}.svg`;
        }
        ...
        if (initialSource === "catalog") {
          catalogWrap.classList.remove("hidden");
          iconName.value = s.icon || "";
        }
        if (initialSource === "url") {
          iconUrlWrap.classList.remove("hidden");
          iconUrlInput.value = s.icon || "";
        }
        if (initialSource === "upload") {
          iconUploadWrap.classList.remove("hidden");
        }
        ...
        fileInput.addEventListener("change", (e) =>
          updateFromUpload(e.target.files?.[0]),
        );
        ...
        function openPlatformPicker() {
          openPicker({
            ...
            onSelect: (i) => {
              ...
              if (!plat) return;
              window.__OPEN_PLATFORM_MODAL__(
                plat,
                ({ handle, country, path }) => {
                  let final = "";
        ...
        const actionWrap = el("div", { class: "flex items-center gap-2" });
        actionWrap.append(saveBtn, cancelBtn);
      ...
      dupBtn.addEventListener("click", () => {
        socials.splice(idx + 1, 0, JSON.parse(JSON.stringify(s)));
        renderSocial({ container, addBtn, socials, scheduleAutoSave });
        scheduleAutoSave();
      });
...
export function renderCategories({
  ...
  if (!Array.isArray(categories) || categories.length === 0) {
    ...
    categories.forEach((c, idx) => {
      const row = el("div", {
        class:
          "group flex items-center gap-3 p-3 rounded-xl bg-slate-950 border border-slate-800 hover:border-slate-700 transition-all",
      });
      row.dataset.dragIndex = String(idx);
      ...
      const gripBtn = el("button", {
        type: "button",
        class:
          "h-8 w-8 inline-flex items-center justify-center rounded-lg bg-slate-900 border border-slate-800 text-slate-500 hover:text-slate-300 hover:bg-slate-800 transition-colors cursor-grab active:cursor-grabbing",
        title: "Déplacer",
        "aria-label": "Déplacer",
      });
      ...
      const nameInput = el("input", {
        ...
        value: catName,
        class:
          "w-full bg-transparent border-none focus:ring-0 text-sm font-medium text-slate-200 placeholder-slate-600 p-0",
      ...
      const count = links
        ? links.filter(
            (l) => l.categoryId === c.id || (c.name && l.categoryId === c.name),
          ).length
        : 0;
      const countBadge = el("span", {
        class: "text-xs text-slate-500",
        text: `${count} lien${count > 1 ? "s" : ""}`,
      });
...
export function renderLinks({
  ...
  const modalSchemeMenu =
    modal && document.getElementById("linkModalSchemeMenu");
  ...
  function openLinkModal(idx) {
    ...
    try {
      ...
      try {
        if (modal) modal.dataset.urlScheme = scheme;
      } catch (err) {
        console.warn("Failed to set modal urlScheme dataset:", err);
      }
      try {
        if (modalSchemeBtn)
          modalSchemeBtn.querySelector("#linkModalSchemeLabel").textContent =
            scheme + "://";
      } catch (err) {
        console.warn("Failed to set modal scheme label:", err);
      }
    } catch {
    ...
    try {
        ...
        if (!presetsWrap._bound) {
            ...
            seeAll.addEventListener("click", () => {
              ...
              openPicker({
                ...
                renderCard: (item) => {
                  ...
                  card.type = "button";
                  card.className =
                    "p-3 rounded border border-slate-700 bg-slate-900 hover:bg-slate-800 text-left flex items-center gap-3";
                    ...
                    window.__OPEN_PLATFORM_MODAL__(
                      ...
                      ({ handle, country, path }) => {
                        ...
                        if (plat.id === "apple-music") {
                          ...
                          const p = (path || "").trim().replace(/^\/+/, "");
                          if (c && p)
                            final = `https://music.apple.com/${c}/${p}`;
                        } else if (plat.id === "apple-podcasts") {
                          const c = (country || "fr").trim();
                          const p = (path || "").trim().replace(/^\/+/, "");
                          if (c && p)
                            final = `https://podcasts.apple.com/${c}/${p}`;
                        } else {
                          const h = (handle || "").trim();
                          final = plat.pattern.replace("{handle}", h);
                        }
                        if (final) {
                          ...
                          try {
                            modal.dataset.urlScheme = /^http:/.test(final)
                              ? "http"
                              : "https";
                          } catch (err) {
                            console.warn(
                              "Failed to update urlScheme dataset:",
                              err,
                            );
                          }
                          const iconSourceEl = document.getElementById(
                            "linkModalIconSource",
                          );
                          const iconInput =
                            document.getElementById("linkModalIconInput");
                          if (iconSourceEl) iconSourceEl.value = "catalog";
                          if (iconInput)
                            iconInput.value = `https://cdn.plinkk.fr/icons/${plat.iconSlug}.svg`;
                        }
                      },
                    );
                  } catch (e) {
                    console.warn("Failed to open platform modal:", e);
                  }
                },
              });
            });
          ...
          platforms.slice(0, 24).forEach((plat) => {
            const btn = el("button", {
              type: "button",
              class:
                "flex flex-col items-center gap-1 p-2 rounded border border-slate-700 bg-gradient-to-b from-slate-800 to-slate-900 hover:from-slate-700 hover:to-slate-800 text-center text-xs transition",
              title: plat.name,
            });
            const img = el("img", {
              src: `https://cdn.plinkk.fr/icons/${plat.iconSlug}.svg`,
              class: "h-7 w-7",
              loading: "lazy",
            });
            const label = el("div", {
              class: "text-xs text-slate-300 truncate w-full",
              text: plat.name,
            });
      ...
      console.warn("Social mode presets initialization failed:", e);
    }

    modalIcon =
      document.getElementById("linkModalIconInput") ||
      document.getElementById("linkModalIconInputUrl");
    ...
    try {
      ...
      if (formMsg)
        formMsg.value = (l.formData && l.formData.successMessage) || "";

      if (typeSel) typeSel.dispatchEvent(new Event("change"));
    } catch (e) {
      console.warn("Advanced fields initialization failed:", e);
    }

    // Set active type and trigger UI update
    try {
      const currentType = l.type || "LINK";
      const typeBtn = modal.querySelector(
        `button[data-type-select="${currentType}"]`,
      );
      ...
      console.warn("Active type initialization failed:", e);
    }
    // Persist editing index on modal element so the save handler (bound once) can read it
    try {
      if (editingIndex !== null)
        modal.dataset.editingIndex = String(editingIndex);
      else delete modal.dataset.editingIndex;
    } catch (err) {
      console.warn("Failed to update editingIndex dataset:", err);
    }
    ...
    try {
      ...
      console.warn("Failed to update icon input readonly state:", err);
    }
    modal.classList.remove("hidden");
    modal.setAttribute("aria-hidden", "false");
    try {
      modalTitle && modalTitle.focus();
    } catch (err) {
      console.warn("Failed to focus modal title:", err);
    }
  }
  ...
  function closeLinkModal() {
    ...
    try {
      if (modal) delete modal.dataset.editingIndex;
    } catch (err) {
      console.warn("Failed to delete editingIndex dataset:", err);
    }
    editingIndex = null;
  }
  ...
  try {
    window.__OPEN_LINK_MODAL__ = (idx, opts) => {
      try {
        if (modal) modal.dataset.mode = "link";
      } catch (e) {
        console.warn("Failed to set modal mode to link:", e);
      }
      openLinkModal(idx);
      if (!opts) return;
      try {
        ...
        if (p.url !== undefined) {
          ...
          try {
            modal.dataset.urlScheme =
              p.url && /^http:/.test(p.url) ? "http" : "https";
          } catch (err) {
            console.warn("Failed to update urlScheme dataset:", err);
          }
          try {
            modalSchemeBtn &&
              (modalSchemeBtn.querySelector(
                "#linkModalSchemeLabel",
              ).textContent = (modal.dataset.urlScheme || "https") + "://");
          } catch (err) {
            console.warn("Failed to update scheme label:", err);
          }
        }
        if (p.icon !== undefined) {
          const iconSourceEl = document.getElementById("linkModalIconSource");
          ...
          if (iconSourceEl) {
            let src = "catalog";
            if (String(p.icon).startsWith("data:")) src = "upload";
            else if (/https?:\/\//.test(p.icon)) src = "url";
            iconSourceEl.value = src;
            if (src === "catalog") {
              if (iconInput) iconInput.value = p.icon;
            }
            if (src === "url" || src === "upload") {
              if (iconUrlInput) iconUrlInput.value = p.icon;
            }
            const iconPickerWrap = document.getElementById(
              "linkModalIconPickerWrap",
            );
            const iconUrlWrap = document.getElementById("linkModalIconUrlWrap");
            const iconUploadWrap = document.getElementById(
              "linkModalIconUploadWrap",
            );
            if (iconPickerWrap)
              iconPickerWrap.classList.toggle(
                "hidden",
                iconSourceEl.value !== "catalog",
              );
            if (iconUrlWrap)
              iconUrlWrap.classList.toggle(
                "hidden",
                iconSourceEl.value !== "url",
              );
            if (iconUploadWrap)
              iconUploadWrap.classList.toggle(
                "hidden",
                iconSourceEl.value !== "upload",
              );
          }
        }
        if (p.description !== undefined) modalDesc.value = p.description || "";
        if (p.categoryId !== undefined && modalCat)
          modalCat.value = p.categoryId || "";
        if (p.newTab !== undefined && modalNewTab)
          modalNewTab.checked = !!p.newTab;
      } catch (e) {
  ...
  try {
    window.__OPEN_SOCIAL_MODAL__ = (idx, opts) => {
      try {
        if (modal) modal.dataset.mode = "social";
      } catch (e) {
        console.warn("Failed to set modal mode to social:", e);
      }
      openLinkModal(idx);
      if (!opts) return;
      try {
  ...
  try {
    ...
    const adjustSocialModeUI = () => {
      ...
      try {
        const header =
          modal.querySelector("h2") || modal.querySelector(".modal-title");
        if (header)
          header.textContent = isSocial
      ...
      try {
        ...
        if (titleEl && titleEl.parentElement)
          titleEl.parentElement.style.display = isSocial ? "none" : "";
      } catch (e) {
        console.error("Caught error", e);
      }
      try {
        ...
        if (desc && desc.parentElement)
          desc.parentElement.style.display = isSocial ? "none" : "";
      } catch (e) {
        console.error("Caught error", e);
      }
      try {
        ...
        if (cat && cat.parentElement)
          cat.parentElement.style.display = isSocial ? "none" : "";
      } catch (e) {
        console.error("Caught error", e);
      }
      ...
      try {
        ...
        if (newTab && newTab.parentElement)
          newTab.parentElement.style.display = isSocial ? "" : "";
      } catch (e) {
        console.error("Caught error", e);
      }
      ...
      try {
        ...
        const urlLabel = modalContent.querySelector(
          "label:has(#linkModalUrlInput)",
        );
        if (!urlLabel) return;
        let presetsBtn = document.getElementById("linkModalPresetsBtn");
        if (isSocial) {
          if (!presetsBtn) {
            presetsBtn = el("button", {
              type: "button",
              id: "linkModalPresetsBtn",
              class:
                "w-full px-3 py-2 rounded bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm font-medium",
              text: "Presets",
            });
            ...
            presetsBtn.addEventListener("click", (ev) => {
              ...
              const content = el("div", {
                class:
                  "relative mx-auto mt-16 w-[92vw] max-w-5xl rounded-lg border border-slate-800 bg-slate-900 shadow-xl animate-pop-in",
              });
              const header = el("div", {
                class:
              ...
              header.append(
                el("h3", {
                  class: "font-semibold",
                  text: "Choisir une plateforme",
                }),
              );
              const closeBtn = el("button", {
                class: "size-8 rounded bg-slate-800 hover:bg-slate-700",
                text: "✕",
              });
              ...
              const searchInput = el("input", {
                ...
                class:
                  "w-full px-3 py-2 rounded bg-slate-800 border border-slate-700 text-slate-200",
              });
              body.append(searchInput);
              const grid = el("div", {
                class: "grid grid-cols-6 gap-2 w-full max-h-96 overflow-auto",
              });
              const platforms = window.__SOCIAL_PLATFORM_LIST__ || [];
              const platformBtns = [];
              platforms.slice(0, 24).forEach((plat) => {
                const btn = el("button", {
                  ...
                  class:
                    "flex flex-col items-center gap-1 p-2 rounded border border-slate-700 bg-gradient-to-b from-slate-800 to-slate-900 hover:from-slate-700 hover:to-slate-800 text-center text-xs transition",
                  title: plat.name,
                  "data-name": plat.name.toLowerCase(),
                });
                const img = el("img", {
                  src: `https://cdn.plinkk.fr/icons/${plat.iconSlug}.svg`,
                  class: "h-7 w-7",
                  ...
                  try {
                    window.__OPEN_PLATFORM_MODAL__(
                      ...
                      ({ handle, country, path }) => {
                        ...
                        if (final) {
                          const urlEl =
                            document.getElementById("linkModalUrlInput");
              ...
              searchInput.addEventListener("keydown", (e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  const firstVisible = platformBtns.find(
        ...
        console.warn("Presets UI adjustment failed:", e);
      }
      ...
      try {
        ...
        console.warn("Input keydown binding failed:", e);
      }
    };
    ...
    openLinkModal = function (idx) {
      oldOpen(idx);
      try {
        adjustSocialModeUI();
      } catch (e) {
        console.warn("adjustSocialModeUI failed on open:", e);
      }
    };
    ...
    closeLinkModal = function () {
      try {
        ...
        console.warn("Failed to delete modal mode dataset:", e);
      }
      try {
        adjustSocialModeUI();
      } catch (e) {
        console.warn("adjustSocialModeUI failed on close:", e);
      }
      oldClose();
    };
  } catch (e) {
    console.warn("Modal patching failed:", e);
  }
  ...
  if (modal && !modal._initialized) {
    ...
    if (modalSchemeBtn && modalSchemeMenu && !modalSchemeBtn._bound) {
      ...
      // choose scheme
      Array.from(
        modalSchemeMenu.querySelectorAll("button[data-scheme]"),
      ).forEach((b) => {
        b.addEventListener("click", (ev) => {
          const s = ev.currentTarget.getAttribute("data-scheme");
          try {
            modal.dataset.urlScheme = s;
          } catch (err) {
            console.warn("Failed to set urlScheme dataset:", err);
          }
          try {
            modalSchemeBtn.querySelector("#linkModalSchemeLabel").textContent =
              s + "://";
          } catch (err) {
            console.warn("Failed to set scheme label:", err);
          }
          modalSchemeMenu.classList.add("hidden");
          // focus url input
          try {
            modalUrl && modalUrl.focus();
          } catch (err) {
            console.warn("Failed to focus modal url input:", err);
          }
        });
      });
      ...
      document.addEventListener("click", () => {
        try {
          ...
          console.warn("Failed to hide scheme menu:", err);
        }
      });
    ...
    if (modalUrl && !modalUrl._schemeBound) {
      modalUrl.addEventListener("input", (e) => {
        try {
          ...
          if (/^https?:\/\//i.test(v)) {
            ...
            if (match) {
              ...
              try {
                ...
                console.warn("Failed to auto-detect urlScheme dataset:", err);
              }
              try {
                modalSchemeBtn &&
                  (modalSchemeBtn.querySelector(
          ...
          console.warn("URL input scheme auto-detection failed:", err);
        }
      });
    ...
    if (modalSaveBtn)
      modalSaveBtn.addEventListener("click", () => {
        ...
        let iconValue = "";
        if (iconSourceEl) {
          if (iconSourceEl.value === "catalog")
            iconValue =
              document.getElementById("linkModalIconInput")?.value.trim() || "";
          else if (iconSourceEl.value === "url")
            iconValue =
              document.getElementById("linkModalIconInputUrl")?.value.trim() ||
              "";
          else if (iconSourceEl.value === "upload")
            iconValue =
              document.getElementById("linkModalIconInputUrl")?.value.trim() ||
              "";
        } else {
        ...
        try {
          scheme =
            modal && modal.dataset && modal.dataset.urlScheme
              ? modal.dataset.urlScheme
              : "https";
        } catch (err) {
          console.warn(
            "Failed to get urlScheme from dataset, defaulting to https:",
            err,
          );
          scheme = "https";
        }
        ...
        const payload = {
          ...
          androidUrl:
            document.getElementById("linkModalAndroidUrl")?.value || null,
          forceAppOpen:
            document.getElementById("linkModalForceAppOpen")?.checked || false,
          clickLimit: document.getElementById("linkModalClickLimit")?.value
            ? parseInt(document.getElementById("linkModalClickLimit").value)
            : null,
        ...
        if (payload.type === "FORM") {
          payload.formData = {
            buttonText:
              document.getElementById("linkModalFormBtnText")?.value ||
              "Envoyer",
            successMessage:
              document.getElementById("linkModalFormSuccessMsg")?.value ||
              "Message envoyé !",
            fields: [
              { type: "text", label: "Nom" },
        ...
        try {
          ...
          console.warn("External save handler logic failed:", e);
        }
        // read editing index from modal dataset (safe across re-renders)
        let mi = null;
        try {
          mi =
            modal && modal.dataset && modal.dataset.editingIndex
              ? parseInt(modal.dataset.editingIndex, 10)
              : null;
        } catch (err) {
    ...
    try {
      ...
      console.warn("Modal backdrop click binding failed:", e);
    }
    // close on Escape
  ...
  links.forEach((l, idx) => {
    ...
    const grip = el("button", {
      type: "button",
      class:
        "h-8 w-8 inline-flex items-center justify-center rounded bg-slate-800 border border-slate-700 hover:bg-slate-700 text-slate-400 cursor-grab touch-none",
      title: "Déplacer",
    ...
    if (!displayIcon) {
      ...
      if (type === "EMBED") {
        ...
        if (url.includes("youtube.com") || url.includes("youtu.be"))
          displayIcon = "https://cdn.plinkk.fr/icons/youtube-alt.svg";
        else if (url.includes("spotify.com"))
          displayIcon = "https://cdn.plinkk.fr/icons/spotify.svg";
    ...
    title.textContent = l.text || l.name || "";
    ...
    const badge = el("span", {
      class: `px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${b.class}`,
      text: b.text,
    });
    ...
    const small = el("div", {
      class: "text-xs text-slate-400 truncate opacity-60",
    });
    ...
    if (l.enabled === false) toggleInput.checked = false;
    else toggleInput.checked = true;
    const toggleBtn = el("button", {
      type: "button",
      class: toggleInput.checked
        ? "px-3 py-1 rounded text-xs bg-emerald-700 text-white"
        : "px-3 py-1 rounded text-xs bg-slate-800 text-slate-400",
    ...
    dupBtn.textContent = "Dupliquer";
    dupBtn.addEventListener("click", () => {
      links.splice(idx + 1, 0, JSON.parse(JSON.stringify(l)));
      renderLinks({ container, addBtn, links, categories, scheduleAutoSave });
      scheduleAutoSave();
    });
...
export function renderLayout({ container, order, scheduleAutoSave, cfg = {} }) {
  ...
  const storeState =
    (window.__EDITOR_STORE__ && window.__EDITOR_STORE__.get()) || {};
  const current = cfg || {};
  ...
  const check = (key) => {
    ...
    for (const src of sources) {
      ...
      if (key === "social") {
        ...
        if (Array.isArray(settings.labels) && settings.labels.length > 0)
          return true;
      } else if (key === "links") {
  ...
  normalized.forEach((key, idx) => {
    ...
    const row = el("div", {
      class:
        "flex items-center gap-3 w-full rounded-xl border border-slate-800 bg-slate-900/50 px-3 py-3 select-none hover:border-slate-700 transition-colors" +
    ...
    const gripBtn = el("button", {
      type: "button",
      class:
        "h-8 w-8 inline-flex items-center justify-center rounded-lg bg-slate-800 border border-slate-700 hover:bg-slate-700 text-slate-400 cursor-grab active:cursor-grabbing transition-colors touch-none",
      title: "Déplacer",
      "aria-label": "Déplacer",
      style: "touch-action: none;",
    });
    ...
    if (!isVisible) {
      const note = el("span", {
        class: "ml-auto text-xs text-slate-400 italic",
        text: "(masqué)",
      });
      row.append(note);
    }
    ...
    try {
      enableDragHandle(
        ...
        container,
        order,
        () => renderLayout({ container, order, scheduleAutoSave, cfg }),
        scheduleAutoSave,
      );
    } catch (err) {
